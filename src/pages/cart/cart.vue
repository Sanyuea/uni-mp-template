...
const isSelectedAll = computed(() => {
  return carList.value.length && carList.value.every((v) => v.selected)
})
...
const selectedCartList = computed(() => {
  return carList.value.filter((v) => v.selected)
})
...
onShow(() => {
  if (memberStore.profile) getMemberCarData()
})
</script>
  ...
  <scroll-view scroll-y class="scroll-view">
    ...
    <template v-if="memberStore.profile">
      ...
      <view class="cart-list" v-if="true">
        ...
        <uni-swipe-action>
          <!-- 滑动操作项 -->
          <uni-swipe-action-item v-for="item in carList" :key="item.id" class="cart-swipe">
            <!-- 商品信息 -->
            <view class="goods">
              <!-- 选中状态 -->
